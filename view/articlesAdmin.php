  ...
  <table><div id="haut"></div>
    <?php foreach ($articles as $article): ?>
      <tr>
          ...
          <td style="width:15%"><a href="index.php?path=article&id=<?php echo $article->getId(); ?>#haut"><?php echo $article -> getTitle(); ?></a></td>
          <td><?php echo $article -> getResume(); ?></td>
          <td style="width:15%">
            <form method="post" action="index.php?path=modifierArticle#haut">
              <input type="hidden" name="id" value="<?php echo $article->getId(); ?>"/>
              <input class="submitBillet" type="submit" value="&plusmn;" title="Modifier">
            </form>
            <form method="post" action="index.php?path=supprimerArticle#haut">
              <input type="hidden" name="id" value="<?php echo $article->getId(); ?>"/>
